...
def preprocess_SBHAR(dataset_path, window, stride):
    ...
    # get labels first then for wach acc file make labels for them
    ...
    for file in file_list:
        if 'acc' in file:
            ...
            data = np.array(pd.read_csv(load_dataset_path + file, delim_whitespace=True, header=None))# 11601 3
            ...
            # normalization, bounds are very similar to that of UCI HAR but with slightly smaller range
            ...
            data = 2 * (data - lower_bound) / diff - 1 # need to keep the last dimension as the channel dimension for bradcasted deduction

            # generate labels first use -1 then put activities in them
            label = np.ones(len(data)) * -1
            for act_seg in act_record[exp_num]:
                label[int(act_seg[1]-1):int(act_seg[2])] = act_seg[0] - 1 # label 0-11

            # sliding window
            data    = sliding_window( data, (window, channel_num), (stride, 1) ) # (3763, 200, 27)
            label   = sliding_window( label, window, stride )
            label   = stats.mode( label, axis=1 )[0][:,0]# choose the most common value as label

            invalid_idx = np.nonzero( label < 0 )[0]# remove non-interested time windows (label==-1)
